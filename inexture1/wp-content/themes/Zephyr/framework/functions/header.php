		...
		foreach ( array( 'default', 'tablets', 'mobiles' ) as $state ) {
			if ( us_arr_path( $header_settings, $state . '.options.orientation' ) == 'hor' ) {
				if ( rwmb_meta( 'us_header_bg' ) != '' ) {
					$header_settings[ $state ]['options']['transparent'] = intval( rwmb_meta( 'us_header_bg' ) == 'transparent' );
				}
				if ( rwmb_meta( 'us_header_pos' ) != '' ) {
					$header_settings[ $state ]['options']['sticky'] = intval( rwmb_meta( 'us_header_pos' ) == 'fixed' );
				}
			}
		}
	}
...
function us_get_header_elm_defaults( $type ) {
	...
	if ( ! isset( $us_header_elm_defaults[ $type ] ) ) {
		foreach ( us_config( 'header-settings.elements.' . $type . '.params', array() ) as $field_name => $field ) {
			...
			if ( substr( $value, 0, 1 ) == '=' AND isset( $usof_options[ substr( $value, 1 ) ] ) ) {
