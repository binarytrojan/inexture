 ...
 *
 * @var $layout_type string Blog layout: classic / masonry / tiles / cards / smallcircle / smallsquare / compact / related / latest
...
if ( $has_preview AND ! post_password_required() ) {
	...
	if ( $columns > 1 AND $layout_type == 'classic' ) {
		$thumbnail_size = 'tnail-3x2';
	}
	$featured_image = has_post_thumbnail() ? get_the_post_thumbnail( get_the_ID(), $thumbnail_size ) : '';
	if ( $featured_image == '' ) {
		// We fetch previews for images at any layout and for any post formats at classic / masonry layouts
	...
	if ( in_array( $layout_type, array( 'classic', 'masonry' ) ) || ( $layout_type == 'cards' AND $post_format == 'gallery' ) ) {
...
<div <?php post_class( $post_classes ) ?> data-id="<?php the_ID() ?>" data-categories="<?php echo implode( ',', $categories_slugs ) ?>">
...
<?php if ( $has_preview AND empty( $featured_html ) ): ?>
		<a href="<?php the_permalink() ?>"<?php if ( $post_format == 'link' ) { echo ' target="_blank"'; }?>>
			<div class="w-blog-post-preview">
...
<?php endif/*( empty( $featured_html ) )*/; ?>
		<div class="w-blog-post-body">
			<h2 class="w-blog-post-title"<?php if ( $title_size != '' ) { echo 'style="font-size: ' . $title_size . '"'; }?>>
				<a class="entry-title" rel="bookmark" href="<?php the_permalink() ?>"<?php if ( $post_format == 'link' ) { echo ' target="_blank"'; }?>><?php the_title(); ?></a>
			</h2>
...
<?php if ( $show_read_more ): ?>
			<a class="w-blog-post-more w-btn" href="<?php the_permalink() ?>"<?php if ( $post_format == 'link' ) { echo ' target="_blank"'; }?>><span class="w-btn-label"><?php _e( 'Read More', 'us' ) ?></span></a>
