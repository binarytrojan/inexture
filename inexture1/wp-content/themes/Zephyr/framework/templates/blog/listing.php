...
if ( ( ! empty( $filter_html ) AND ( $pagination == 'none' OR $wp_query->max_num_pages == 1 ) ) OR ( in_array( $layout_type, array( 'masonry', 'cards' ) ) AND $columns > 1 ) ) {
	// We'll need the isotope script for masonry and cards layouts
	wp_enqueue_script( 'us-isotope' );
	$classes .= ' with_isotope';

	if ( ! in_array( $layout_type, array( 'masonry', 'cards' ) ) ) {
		$classes .= ' isotope_fit_rows';
	}
}


?><div class="w-blog<?php echo $classes ?>"><?php
echo $filter_html;
